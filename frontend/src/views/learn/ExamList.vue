<template>
  <div class="page-container">
    ...
    <el-table :data="list" style="margin-top: 16px;">
      ...
      <el-table-column label="操作" width="160">
        <template #default="{ row }">
          <el-button type="primary" link @click="start(row)">进入</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialog" title="答题" width="700px">
      <div v-if="exam">
        <div v-for="(q, i) in exam.questions" :key="q.id" style="margin-bottom: 16px;">
        ...
        <el-button type="success" :loading="loading" @click="submit">交卷</el-button>
...
async function load() {
  const res = await request.get('/exams', { params: { current: current.value, size: size.value } })
  list.value = res.data.records
  total.value = res.data.total
}
async function start(row) {
  exam.value = (await request.get(`/exams/${row.id}`)).data
  answers.value = {}
  result.value = null
  const res = await request.post(`/exams/${row.id}/start`)
...
async function submit() {
  loading.value = true
  try {
    const res = await request.post(`/exams/attempts/${attemptId.value}/submit`, { answers: answers.value })
